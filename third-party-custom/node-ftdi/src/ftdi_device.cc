 ...
 public:
  ...
  void HandleOKCallback () {
    ...
    if(status != FT_OK || (baton->length != 0))
    {
      ...
      // Commit cad0453f53 changes Integer to Number below
      Handle<Value> constructorArgs[3] = { slowBuffer, Nan::New<Number>(baton->length), Nan::New<Number>(0) };
      // NKS
      Isolate* isolate = Isolate::GetCurrent();
      Local<Context> context = isolate->GetCurrentContext();
      // NKS below was: 3, constructorArgs
      Local<Object> actualBuffer = bufferConstructor->NewInstance(context, 3, constructorArgs).ToLocalChecked();
      argv[1] = actualBuffer;
...
FT_STATUS FtdiDevice::OpenAsync(FtdiDevice* device, Nan::Callback *callback_read)
{
  ...
  else
  {
    ...
    if (ftStatus != FT_OK)
    {
      fprintf(stderr, "Can't Set DeviceSettings: %s, but if you're using the device in 245 (etc) mode, this may run just fine, since execution is allowed to continue ...\n", error_strings[ftStatus]);
      // NOTE WARNING: Here testing when just plain opening without baud,
      // eg for async ft245 style fifo writes into the host from board
      // so reset ftStatus to ok, even if the settings didn't work -- because
      // we excluded them
      ftStatus = FT_OK;
    }
  }
...
FT_STATUS FtdiDevice::SetDeviceSettings()
{
  ...
  // Hehe you better put that trailing \n in the fprintf or call fflush()
  // Anyway, something about using multiple strings here is broken or incorrectly called
  //fprintf(stderr, "deviceParams: baudRate: %d\n", deviceParams.baudRate); // stopBits: %s parity: %s baudRate: %s bitMask: %s bitMode: %s\n", deviceParams.wordLength, deviceParams.stopBits, deviceParams.parity, deviceParams.baudRate, deviceParams.bitMask, deviceParams.bitMode);
...
void FtdiDevice::ExtractDeviceSettings(Local<Object> options)
{
  ...
  // OB
  //printf("ExtractDeviceSettings: baudrate from options: %s\n", Nan::Get(options, baudrate));
  //Nan::JSON NanJSON;

  //printf("ExtractDeviceSettings: baudrate from options: %s\n", NanJSON.Stringify(options));

  //Nan::ThrowError("test error");

  if(options->Has(baudrate))
  { // was Get(baudrate)->ToInt32()->Int32Value()
    deviceParams.baudRate = options->Get(baudrate)->Int32Value();
  }
  if(options->Has(databits))
  { // so was this
    deviceParams.wordLength = GetWordLength(options->Get(databits)->Int32Value());
  }
  if(options->Has(stopbits))
  { // so was this
    deviceParams.stopBits = GetStopBits(options->Get(stopbits)->Int32Value());
  }
  ...
  { // so was this
      deviceParams.bitMode = options->Get(bitmode)->Int32Value();
      hasBitSettings = true;
  } else {
  ...
  { // so was this
      deviceParams.bitMask = options->Get(bitmask)->Int32Value();
      hasBitSettings = true;
  }
